...
### v0.20.1 — ссылка на репозиторий в подвале

- Строка версии внизу дашборда стала `v0.20.1 · GitHub · MIT`: код открытый,
  и номер версии — самое естественное место, откуда до репозитория один клик
- `src/repoLink.ts` собирает эту строку из `package.json`, а не из константы в
  разметке: `repository` приводится к https-виду (`git+`, `.git`, scp-форма
  `git@host:user/repo`, шорткаты `github:user/repo` и `user/repo`), всё, что не
  свелось к http(s), отбрасывается — в `href` не должен попасть `javascript:`.
  Ярлык ссылки — по хосту (GitHub/GitLab/Bitbucket/Codeberg, иначе хостнейм),
  лицензия — из поля `license`. Разделитель ` · ` печатается только вместе с
  тем, что за ним следует
- Ссылка наследует приглушённый вид подвала и подсвечивается только на ховере;
  внешний браузер открывает сам VSCode — обработчик в вебвью не нужен
- Тег релиза теперь ставит `hooks/post-commit` при бампе `version` — см.
  «Цикл релиза»; заодно синхронизирован `package-lock.json`, застрявший на
  `0.17.1`
...
4. `git commit -m "feat(vX.Y.Z): ..."` — тег `vX.Y.Z` навешивает хук
   `hooks/post-commit`, как только в коммите поменялся `version`.
5. `git push origin master` — `push.followTags` уносит тег вместе с коммитом.
6. (после v0.9.x) GitHub Release + автоматический `vsce publish`.

Теги больше не забываются: `npm install` (через `prepare` →
`scripts/install-git-hooks.js`) ставит `core.hooksPath=hooks` и
`push.followTags=true`, `hooks/post-commit` сравнивает `version` в `HEAD` и
`HEAD~1` и вешает **аннотированный** `vX.Y.Z` с темой коммита в качестве
сообщения. Лёгкий тег тут не годится: `--follow-tags` и `push.followTags`
молча игнорируют лёгкие теги. Существующий тег хук не двигает — только ругается
в stderr, если он указывает на другой коммит (amend/cherry-pick/ребейз).
Заодно ожили лежавшие рядом `hooks/pre-commit` и `hooks/commit-msg` — раньше
`core.hooksPath` не был выставлен и они просто не вызывались.

Долг по тегам закрыт вручную 2026-08-14: `v0.14.0`–`v0.18.1` и `v0.20.0`
навешены задним числом (лёгкие, на первый коммит с этой версией) и запушены.
Ранние `v0.1.0`–`v0.4.0` и `v0.9.0` решено не тегать.
