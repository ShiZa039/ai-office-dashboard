...
var MESSAGES = {
  en: {
    ...
    labelErrors: "errors",
    labelTeam: "team",
    titleTeam: "Every agent this project knows: the roster from the agent directories plus anyone seen working here. It changes when agents are added, not when they run.",
    labelCompleted: "completed",
    labelTokens: "Tokens",
    tokensSession: "session",
    tokensTotal: "total",
    tokensSessionTitle: "Current CLI session",
    tokensTotalTitle: "Every session ever recorded for this project",
    tokensIn: "incoming (sent to the model)",
    tokensOut: "outgoing (generated by the model)",
    tokensPrompt: "prompt",
    tokensPromptFull: "prompt, full price",
    tokensCacheWrite: "cache write (1.25×/2×)",
    tokensCacheRead: "cache read (0.1×)",
  ...
  ru: {
    ...
    labelErrors: "ошибки",
    labelTeam: "команда",
    titleTeam: "Все агенты, которых знает проект: ростер из каталогов агентов плюс все, кого видели здесь за работой. Меняется при добавлении агентов, а не при их запусках.",
    labelCompleted: "завершено",
    labelTokens: "Токены",
    tokensSession: "сессия",
    tokensTotal: "всего",
    tokensSessionTitle: "Текущая сессия CLI",
    tokensTotalTitle: "Все сессии этого проекта за всё время",
    tokensIn: "входящие (ушли в модель)",
    tokensOut: "исходящие (сгенерированы моделью)",
    tokensPrompt: "запрос",
    tokensPromptFull: "запрос, полная цена",
    tokensCacheWrite: "запись кэша (1.25×/2×)",
    tokensCacheRead: "чтение кэша (0.1×)",
...
/** 942 → "942", 288684 → "288.7K", 3861352366 → "3.86B". */
function formatTokens(n) {
  ...
  for (var i = 0; i < units.length; i++) {
    if (n >= units[i][0]) {
      var v = n / units[i][0];
      var s = v >= 100 ? v.toFixed(0) : v.toFixed(v >= 10 ? 1 : 2);
      // Trim "9.40" → "9.4" and "1.00" → "1", but never touch a plain "200".
...
/** Exact count with thin-space grouping, for the tooltips. */
function groupDigits(n) {
  return String(Math.round(Number(n) || 0)).replace(/\B(?=(\d{3})+(?!\d))/g, " ");
}

/**
 * One scope's pair of cells. "Incoming" is prompt + both kinds of cache
 ...
 */
function fillTokenCells(inId, outId, totals, scopeTitle, byModel, byAgent, costParts) {
  ...
  // Tokens and money on the same row: cache traffic is most of the volume and
  // a sliver of the bill, which is only visible when both are side by side.
  var parts = costParts || null;
  var incomingCost = parts
    ? (parts.input || 0) + (parts.cacheWrite || 0) + (parts.cacheRead || 0)
    : null;
  inEl.title =
    scopeTitle + "\n" + tr("tokensIn") + ": " + amount(incoming, incomingCost) +
    "\n  " + tr("tokensPromptFull") + ": " +
      amount(totals.input, parts ? parts.input : null) +
    "\n  " + tr("tokensCacheWrite") + ": " +
      amount(totals.cacheCreate, parts ? parts.cacheWrite : null) +
    "\n  " + tr("tokensCacheRead") + ": " +
      amount(totals.cacheRead, parts ? parts.cacheRead : null) +
    modelBreakdown(byModel, incomingOf) +
    agentBreakdown(byAgent, incomingOf);
  outEl.title =
    scopeTitle + "\n" + tr("tokensOut") + ": " +
      amount(totals.output, parts ? parts.output : null) +
    modelBreakdown(byModel, outputOf) +
    agentBreakdown(byAgent, outputOf);
}

/** "129 213 · ≈ $0.81" — the token count, and what it cost when we know. */
function amount(tokens, cost) {
  var text = groupDigits(tokens);
  if (cost == null || isNaN(cost)) return text;
  return text + " · ≈ " + formatUsdPrecise(cost);
}

/**
 * Money at tooltip resolution: a single prompt can be worth a tenth of a cent,
 * and rounding those rows to "$0.00" would hide exactly the split being shown.
 */
...
function renderTokens(tokens) {
  ...
  fillTokenCells(
    ...
    tokens.session ? tokens.session.costParts : null,
  );
  fillTokenCells(
...
function renderUsage() {
  ...
  // The token panel can be the only live section (quota APIs off / failing),
  // so it also feeds the "updated" stamp instead of leaving it on "loading…".
  var stamp = fetched || (cost && cost.fetchedAt) ||
    (lastUsage.tokens && lastUsage.tokens.fetchedAt);
  if (upd && stamp) upd.textContent = tr("updatedAt", { t: formatTime(stamp) });
}
