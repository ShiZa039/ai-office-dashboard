...
**Current version:** `v0.20.1` · zero-config: dual CLI support (Claude Code + Kimi Code), automatic hook installation, agent auto-discovery, dynamic rooms, real Pro/Max plan limits, token & cost counters (per model and per subagent), auto-stop at plan limits, agent emergency stop, en/ru UI.
...
- **Auto-stop at plan limits** — the same stop trips by itself when any plan limit (Claude or Kimi) crosses the threshold, so a runaway session doesn't eat the whole window. It is a one-shot fuse per limit window: it fires once at the threshold (95% by default) and once more at the final one (99%), then stays quiet until the limit resets, leaving you the remaining percent for a commit and a push. The flag it writes expires with the limit window, so yesterday's auto-stop never greets you today.
...
1. Install the `.vsix`:
   ...
   ```
   code --install-extension ai-office-dashboard-0.20.1.vsix --force
   ```
...
- an **auto-stop** (see above) additionally stamps `until` into the flag — the reset time of the limit that tripped it. Both the hooks and the extension treat an expired flag as released, and the extension deletes the file. A manual stop stays indefinite, and merging flags never shortens it.
...
The bottom-right corner of the dashboard shows the installed version, a link to the repository, and the license — `v0.20.1 · GitHub · MIT`; the link is derived from `package.json`, not hardcoded in the markup.
