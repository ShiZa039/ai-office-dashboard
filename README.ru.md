...
**Текущая версия:** `v0.18.0` · zero-config: поддержка двух CLI (Claude Code + Kimi Code), автоустановка хуков, авто-обнаружение агентов, динамические комнаты, реальные лимиты Pro/Max, счётчики токенов и стоимости, экстренная остановка агентов, UI en/ru.
...
- **Стоимость по ценам API ($)** — третья колонка рядом со счётчиками токенов оценивает сессию и проект по публичным ценам API (запись кэша ×1.25/×2 по TTL, чтение ×0.1), в тултипе — разбивка стоимости по моделям. Ниже — строка сравнения последних 30 дней с ценой подписки: «за 30 дней ≈ $142 по ценам API · Max $100/мес — подписка выгоднее ×1.4». Цена подписки — настройка `aiOffice.usage.subscriptionUsd`, при 0 угадывается по плану. Модели без известной цены (например, Kimi) исключаются, а не считаются нулём.
- **Индикатор контекста** — насколько заполнено контекстное окно текущей сессии: размер промпта последнего запроса против окна модели (200K, для `[1m]`-моделей 1M), пороги 70/90% — жёлтый/красный. Запросы сабагентов (sidechain) не двигают индикатор — у них свой контекст.
- **Спарклайн расхода** — темп токенов сессии (токены/мин) за последние 30 минут: исходящие зелёной областью, входящие (с кэшем) синей линией.
...
| `aiOffice.usage.subscriptionUsd` | `0` | Месячная цена подписки ($) для строки «стоимость по API против подписки»; 0 = угадать по плану (Pro ≈ $20, Max ≈ $100/$200) |
