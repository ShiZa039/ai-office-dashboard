...
**Текущая версия:** `v0.20.1` · zero-config: поддержка двух CLI (Claude Code + Kimi Code), автоустановка хуков, авто-обнаружение агентов, динамические комнаты, реальные лимиты Pro/Max, счётчики токенов и стоимости (по моделям и по субагентам), автостоп по лимитам плана, экстренная остановка агентов, UI en/ru.
...
- **Автостоп по лимитам плана** — та же остановка взводится сама, когда любой лимит (Claude или Kimi) переходит порог, чтобы разогнавшаяся сессия не съела всё окно. Это разовый предохранитель на окно лимита: срабатывание на основном пороге (по умолчанию 95%) и ещё одно, последнее, — на финальном (99%), дальше до сброса окна не беспокоит, и оставшиеся проценты можно потратить на коммит и пуш. Флаг остановки протухает вместе с окном лимита — вчерашний автостоп не встретит вас сегодня.
...
- **Счётчики токенов** — входящие (запрос + запись кэша + чтение кэша) и исходящие токены за текущую сессию CLI и за весь проект, считаются прямо по локальным транскриптам (`~/.claude/projects/`) — без подпроцессов и без сети. Итог по проекту охватывает все сессии за всё время и переживает удаление старых транскриптов. Транскрипты субагентов (`…/<sessionId>/subagents/agent-*.jsonl`) тоже учитываются. При наведении на счётчик — разбивка по видам (запрос / запись кэша / чтение кэша), по моделям (Opus, Sonnet, Haiku из фоновых задач и всё остальное, что записал CLI) и по субагентам — видно, кто именно сжёг бюджет (секция по агентам прячется, когда в скоупе только основная цепочка).
- **Стоимость по ценам API ($)** — третья колонка рядом со счётчиками токенов оценивает сессию и проект по публичным ценам API (запись кэша ×1.25/×2 по TTL, чтение ×0.1), в тултипе — разбивка стоимости по моделям. В каждой строке тултипа и токены, и деньги — `чтение кэша (0.1×): 8 902 917 · ≈ $4.45`: сразу видно, что кэш — это почти весь объём и малая часть счёта. Ниже — строка сравнения последних 30 дней с ценой подписки: «за 30 дней ≈ $142 по ценам API · Max $100/мес — подписка выгоднее ×1.4». Цена подписки — настройка `aiOffice.usage.subscriptionUsd`, при 0 угадывается по плану. Модели без известной цены (например, Kimi) исключаются, а не считаются нулём.
...
- **автостоп** (см. выше) дополнительно проставляет во флаг `until` — время сброса сработавшего лимита. Хуки и расширение считают протухший флаг снятым, расширение заодно удаляет файл. Ручная остановка по-прежнему бессрочная, и слияние флагов никогда её не укорачивает.
...
| `aiOffice.autoStop.enabled` | `true` | Взводить экстренную остановку самостоятельно, когда лимит плана дошёл до порога. Разовый предохранитель на окно лимита: срабатывание на пороге, ещё одно — на финальном пороге, дальше тишина до сброса лимита |
| `aiOffice.autoStop.thresholdPercent` | `95` | Процент выработки лимита плана, на котором взводится автостоп |
| `aiOffice.autoStop.finalThresholdPercent` | `99` | Процент последнего предупреждения в этом окне; равен основному порогу = один-единственный выстрел |
...
В правом нижнем углу дашборда — установленная версия, ссылка на репозиторий и лицензия: `v0.20.1 · GitHub · MIT`; ссылка собирается из `package.json`, а не зашита в разметку.
